        ...
        if ($mtgs<>'' and $mtgs<>'NULL') {
            ...
            foreach ($mtgs as $imtg => $mtg) {
                ...
                if ((strpos(strtoupper($row['locationNotes']), "SUSPENDED") !== false) ||
                    ...
                    $csuspendedMtgs++;
                    $mtg['types'] .= " TC";
                    // Stuffing online meeting information in note2 and note3 fields
                    if($row['note2'] || $row['note3']){
                        $conlineMtgs++;
                        $mtg['types'] .= " ONL";
                        $new_row['mtgURL']=$row['note2'];
                        $new_row['mtgPhone']=$row['note3'];
                    }
                }
...
printf("marked <b>%d</b> meetings as ONLINE<br>",$conlineMtgs);
